export const USER_REQUEST_FAILED = 'USER_REQUEST_FAILED';
export const USER_REGISTRATION_SUCCESS = 'USER_REGISTRATION_SUCCESS';
export const USER_REGISTRATION_REQUEST = 'USER_REGISTRATION_REQUEST';
export const USER_LOGIN_REQUEST = 'USER_LOGIN_REQUEST';
export const USER_LOGIN_SUCCESS = 'USER_LOGIN_SUCCESS';
export const USER_FORGET_PASSWORD_SUCCESS = 'USER_FORGET_PASSWORD_SUCCESS';
export const USER_FORGET_PASSWORD_REQUEST = 'USER_FORGET_PASSWORD_REQUEST';
export const USER_RESET_PASSWORD_SUCCESS = 'USER_RESET_PASSWORD_SUCCESS';
export const USER_RESET_PASSWORD_REQUEST = 'USER_RESET_PASSWORD_REQUEST';
export const GET_PACKAGES_REQUEST = 'GET_PACKAGES_REQUEST';
export const GET_PACKAGES_SUCCESS = 'GET_PACKAGES_SUCCESS';
export const GET_PACKAGES_FAILURE = 'GET_PACKAGES_FAILURE';
export const GET_BOTS_REQUEST = 'GET_BOTS_REQUEST';
export const GET_BOTS_SUCCESS = 'GET_BOTS_SUCCESS';
export const GET_BOTS_FAILURE = 'GET_BOTS_FAILURE';
export const CREATE_ORDER_REQUEST = 'CREATE_ORDER_REQUEST';
export const CREATE_ORDER_SUCCESS = 'CREATE_ORDER_SUCCESS';
export const CREATE_ORDER_FAILURE = 'CREATE_ORDER_FAILURE';
export const RECHARGE_BOT_SUCCESS = 'RECHARGE_BOT_SUCCESS';
export const RECHARGE_BOT_REQUEST = 'RECHARGE_BOT_REQUEST';
export const RECHARGE_BOT_FAILURE = 'RECHARGE_BOT_FAILURE';
export const RETRAIN_BOT_SUCCESS = 'RETRAIN_BOT_SUCCESS';
export const RETRAIN_BOT_REQUEST = 'RETRAIN_BOT_REQUEST';
export const RETRAIN_BOT_FAILURE = 'RETRAIN_BOT_FAILURE';
export const GET_ORDER_REQUEST = 'GET_ORDER_REQUEST';
export const GET_ORDER_SUCCESS = 'GET_ORDER_SUCCESS';
export const GET_ORDER_FAILURE = 'GET_ORDER_FAILURE';
export const GET_PAID_PACKAGES_REQUEST = 'GET_PAID_PACKAGES_REQUEST';
export const GET_PAID_PACKAGES_SUCCESS = 'GET_PAID_PACKAGES_SUCCESS';
export const GET_PAID_PACKAGES_FAILURE = 'GET_PAID_PACKAGES_FAILURE';
export const GET_ALL_ORDERS_REQUEST = 'GET_ALL_ORDERS_REQUEST';
export const GET_ALL_ORDERS_SUCCESS = 'GET_ALL_ORDERS_SUCCESS';
export const GET_ALL_ORDERS_FAILURE = 'GET_ALL_ORDERS_FAILURE';
